...
const ListingPage: React.FC = () => {
    ...
    const [status, updateStatus] = useState<string>('idle')
    const [listingData, updateListingData] = useState<Listing | null>(null)
    ...
    const getTagElements = () => {
        const elements: JSX.Element[] | undefined = listingData?.tags?.map(tag => <StyledTag key={tag.id}>{tag.text}</StyledTag>)
        return elements
    }
