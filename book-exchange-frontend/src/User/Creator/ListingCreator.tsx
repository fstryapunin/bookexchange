...
const InputContainer = styled.div<{fr?: string | number}>`
    display: flex;
...
const ListingCreator: React.FC = () => {
    ...
    const [newTags, updateNewTags] = useState<{text:string}[]>([])
    const [tagNameInput, updateTagNameInput] = useState('')    
    const location = useLocation();
    const dispatch = useAppDispatch();
    const navigate = useNavigate();
    const token = useAppSelector(state => state.user.auth.token)
    const tagsStatus = useAppSelector(state => state.creator.tags.status)
    const tagsData = useAppSelector(state => state.creator.tags.loadedTags)
    const listingImages = useAppSelector(state => state.creator.images)
    ...
    const handleTagClick = (data: (Tag | {text: string}), action: string, custom: (boolean | null)) => {
        if (action === 'select') {
            if (selectedTags.length + newTags.length < 5) {
                if (custom) {
                    ...
                    const nextTags = [...selectedTags, data as Tag]
            ...
            if (custom) {
                ...
                updateSelectedTags(nextTags as Tag[])
            }
        }
    }    

    const getTagElements = (type: string) => {
        if (type === 'menu') {
            if (tagsStatus === 'loaded' || 'loading') {
                ...
                const tags = filteredTags.map(tagObj => {
                    return <ListingTag key={tagObj.id} data={tagObj} handleClick={handleTagClick} selected={false} custom={false}/>
                }) 
    ...
    const getEnableUpload = () => {
        ...
        let property: keyof NewListing;

        for (property in listingObj) { 
    ...
    const handleSuccessfulUpload = (data: Listing) => {
    ...
    const handleCreateClick = async (event: any) => {
        ...
        if (!isEdit) {            
            ...
            const assignedTags: (Tag | {test: string})[] = []
            
            //check if selected tag is in old tags
            selectedTags.forEach(selectedTag => {
                const isOld = data.tags.some((oldTag: Tag) => oldTag.id === selectedTag.id)
                if (!isOld) {
            ...
            const newImages: any[] = []
            const removedImages: ListingImage[] = []
            ...
            data.images.forEach((oldImage: any) => {
