...
const ImageInput: React.FC<IProps> = ({data}) => {    
    const [imagePreviews, updateImagePreviews] = useState<{name: string, src: string}[]>([])    
    const [uiWarning, updateUiWarning] = useState<{display: boolean, text: string | null}>({
        display: false,
        text: null
    })
    const dispatch = useAppDispatch()
    const images = useAppSelector(state => state.creator.images.uploads)    
    ...
    const checkUploads = (files: any) => {
        const checkForDuplicates = (list: any) => {
            const nameList = [...images.map(file => file.name), ...list.map((file: any) => file.name)]
